...
> **Moved out of this repo.** The Dashboard now lives in `marcdt11/dashboard` at
> `dashboard.marctorrence.com`, isolated from the public poker tools site. Its
> backlog (KPIs, charts, filters, tech debt re: hardcoded password + CORS proxies)
> is tracked there.
...
- [x] Hand history navigation (back / forward + branch-and-replay) — `◀`/`▶` buttons in desktop sidebar and mobile control bar step through every reviewable state of the current hand (flop dealt, each villain/hero action, each new street card, hand end). Silent re-render via `structuredClone` snapshots in `game.history[]`; acting while rewound branches (truncates history, reshuffles remaining deck, replays with animation). Action buttons only appear on steps where hero has a live decision (`heroToAct` flag); other steps are review-only. `navLocked` disables nav during animations. New Hand / reroll reseed history.
- [x] All-in runout now renders — `runOutBoard()` previously dealt the remaining turn/river into game state but never called `renderHand()`, so the felt still showed only the pre-all-in board. Now reveals the runout cards with a staggered deal animation (brief pause before + after).
- [x] Reroll pot regression fixed — "New Turn"/"New River" (`rerollStreet`) reset the pot and stacks to start-of-*hand* values (`game.spot.pot`), wiping out earlier streets' betting (e.g. flop bets vanished when rerolling the turn). Now `startStreetAction` captures `streetStartPot`/`streetStartUserStack`/`streetStartOppStack` at the top of each street, and `rerollStreet` restores from those. Added to `SNAPSHOT_FIELDS` so the correct street-start context survives history navigation.
