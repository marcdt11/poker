...
- **Pre-push guard (local):** `.githooks/pre-push` runs `npm test` and blocks the push on failure. Enable once per clone with `git config core.hooksPath .githooks`. Bypass intentionally with `git push --no-verify`. Client-side only — it does **not** guard GitHub web-UI commits or the Pages deploy.
- **CI gate (server-side):** `.github/workflows/test.yml` runs `npm ci && npm test` on every push to `main` and every PR. Unbypassable. Note: this *reports* a failed check but does not by itself block the Pages deploy — to make it a hard merge gate, enable branch protection on `main` in GitHub repo settings requiring the "Tests" check (implies a PR-based workflow, no direct pushes to main).
